...
public class ModularFeatureEditorDialog extends Dialog{
	...
	protected Control createDialogArea(Composite parent) {
	    ...
	    browseProject = new Button(filterGroupComposite, SWT.RADIO);
	    browseProject.setText("Browse Current Project");
	    browseProject.setSelection(true);
	    
	    browseWorkspace = new Button(filterGroupComposite, SWT.RADIO);
	    browseWorkspace.setText("Browse Workspace");
	    browseWorkspace.setSelection(false);
	    ...
	    final TableViewer choiceTableViewer = choicesOfValues == null ? null : new TableViewer(choiceTable);
	...
	private List<?> getChoicesofValues() {
		...
		if(browseProject.getSelection()){
			...
			Resource res = set.createResource(URI.createPlatformResourceURI(fil.getFullPath().toString(), true));
			eval.execute(Platform.getExtensionRegistry(), this.IdExtension ,null,res, (EClass) feature.getEType());
		}
		else
			eval.execute(Platform.getExtensionRegistry(), this.IdExtension ,WTNewProjectNature.ID,null, (EClass) feature.getEType());			
