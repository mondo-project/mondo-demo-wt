...
public class Id2objectMatcher extends BaseMatcher<Id2objectMatch> {
  ...
  protected Id2objectMatch tupleToMatch(final Tuple t) {
    try {
    	return Id2objectMatch.newMatch((EObject) t.get(POSITION_OBJECT), (String) t.get(POSITION_KEY));
  ...
  protected Id2objectMatch arrayToMatch(final Object[] match) {
    try {
    	return Id2objectMatch.newMatch((EObject) match[POSITION_OBJECT], (String) match[POSITION_KEY]);
    } catch(ClassCastException e) {
  ...
  protected Id2objectMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return Id2objectMatch.newMutableMatch((EObject) match[POSITION_OBJECT], (String) match[POSITION_KEY]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
